...
class Layer():
    def __init__(self, layer_number, FLAGS, env, sess, agent_params):
        ...
        print("max steps in env: ", env.max_actions)
        print("step limit per layer: ", self.time_limit)
        print(" ")
    ...
    def get_action(self, state, goal):
        return self.actor.get_action(state.reshape(1, -1), goal.reshape(1, -1))[0]

    def get_value(self, state, goal, action=None):
        if action is None: action = self.get_action(state, goal)
        return self.critic.get_Q_value(state.reshape((1,-1)), goal.reshape((1,-1)), action.reshape((1,-1)))

    def sample_goal(self):
        subgoal = np.zeros((self._env.subgoal_dim))
        # Each dimension of random action should take some value in the dimension's range
        for i in range(len(subgoal)):
            subgoal[i] = np.random.uniform(self._env.subgoal_bounds[i][0],self._env.subgoal_bounds[i][1])
        return self.project_subgoal_to_end_goal(subgoal)

    def project_state_to_end_goal(self, state):
        return self._env.project_state_to_end_goal(self._env.sim, state)

    def project_state_to_subgoal(self, state):
        return self._env.project_state_to_subgoal(self._env.sim, state)
    
    def project_goal_to_state(self, subgoal):
        subgoal = subgoal.squeeze()
        state = self._env.get_state()
        state[0:3] = subgoal[0:3]
        return state

    def project_end_goal_to_subgoal(self, goal):
        subgoal = np.zeros(self._env.subgoal_dim)
        subgoal[0:3] = goal.squeeze()
        return subgoal
